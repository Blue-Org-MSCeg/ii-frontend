...
const IRgeneration = () => {
  ...
  const generatePDF = async () => {
    fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/orders/invoice/${client.businessName}/${startDate}/${endDate}`
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const generateAndSharePDF = async () => {
    if (invoice.length !== 0) {
      ...
      Alert.alert("Validation Error", "No orders found in these dates", [
        { text: "OK" },
      ]);
    }
  };
  ...
  const fetchReportSheetDetails = () => {
    fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/orders/reportSheet/${client.businessName}/${startDate}/${endDate}`
    )
      ...
      .then((data) => {
        ...
        rs.forEach((ele) => {
          ...
          subArray = foodOrder.map((item) =>
            ele.orders.reduce(
              (acc, order) =>
                order.foodItem === item ? acc + order.numberOfHeads : "-",
              0
            )
          );
      ...
      <View className="justify-between items-center mb-28">
        ...
        <View className="py-2 px-4 rounded">
          <Button
            ...
            onPress={fetchReportSheetDetails}
          />
        </View>
      </View>
    </View>
