...
export default function MenuQuotation() {
  ...
  useEffect(() => {
    fetch(`http://192.168.120.16:3000/api/v1/clients/quotation/${client.id}`, {
      method: "PUT",
      headers: {
  ...
  useEffect(() => {
    ...
    if (Object.keys(editFormData).length !== 0) {
      ...
      fetch(
        `http://192.168.197.222:3000/api/v1/clients/quotation/${client.id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteItem = (item) => {
    ...
    fetch(
      `http://192.168.197.222:3000/api/v1/clients/quotation/${client.id}/${item._id}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
    ...
    const updatedMenuQuotation = menuQuotation.filter(
